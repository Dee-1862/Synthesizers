...
print("##### Loading data.....")
target_variable = 'total_UPDRS'
air_quality = fetch_ucirepo(id=189)
...
def synthetic_pipeline(model, X_vals, Y_vals, X_test, y_test, num_synthetic_datasets = 10):
    ...
    for i in tqdm(range(num_synthetic_datasets), desc="Generating synthetic datasets", unit="dataset"):
        ...
        block_print()
        synth_data_no_target = synth.fit_sample(df_no_target, preprocessor_eps = 0.7)
        restore_print()
...
for model_name, model in models.items():
    ...
    results = synthetic_pipeline(model, X_vals, Y_vals, X_test, y_test, num_synthetic_datasets = 10)
...
excel_name = "Parkinsons_Telemonitoring/Aggregate Seeded/PT_AggregateSeeded_Results.xlsx"
