...
df = pd.concat([X, Y.rename(target_variable)], axis=1)
row_cnt = df.shape[0]
...
def synthetic_pipeline(model, X_vals, Y_vals, X_test, y_test, continuous_cols, x_bounds, y_bounds, num_synthetic_datasets=10):
    ...
    for i in tqdm(range(num_synthetic_datasets), desc="Generating synthetic datasets", unit="dataset"):
        ...
        block_print() # Block the print statements from the DPCTGAN training
        ...
        restore_print() # Restore the print statements
...
excel_name = "California_Housing/DPCTGAN/CaliHousing_DPCTGAN_Results.xlsx"
