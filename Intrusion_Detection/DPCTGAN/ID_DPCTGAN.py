...
def synthetic_pipeline(model, X_test, y_test, continuous_cols, num_synthetic_datasets=10):
    ...
    for i in tqdm(range(num_synthetic_datasets), desc="Generating synthetic datasets", unit="dataset"):
        ...
        epsilon_val,
        DPCTGAN(epochs=epochs_val, delta=delta_val, cuda = True, batch_size=90), # Decreased the batch size to 90 from 500 as the dataset size is 184. 
        None
        )
...
excel_name = "Intrusion_Detection/DPCTGAN/ID_DPCTGAN_Results.xlsx"
